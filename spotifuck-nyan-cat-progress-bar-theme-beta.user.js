...
// @updateURL    https://raw.githubusercontent.com/Myst1cX/Nyan-Cat-Progress-Bar-Port/main/spotifuck-nyan-cat-progress-bar-theme-beta.user.js
// @downloadURL  https://raw.githubusercontent.com/Myst1cX/Nyan-Cat-Progress-Bar-Port/main/spotifuck-nyan-cat-progress-bar-theme-beta.user.js
// ==/UserScript==
...
// HARDENED: NATIVE SONG BAR'S RAINBOW FILL NO LONGER DEPENDS ON GUESSING SPOTIFY'S
// DIV STRUCTURE
// applyRainbowToFill() found the song bar's fill by walking [data-testid=
// "progress-bar-background"]'s children, skipping the one tagged
// progress-bar-handle, skipping any child with an inline style.left or
// style.anchorName (assumed to be positioning artifacts rather than the fill), and
// then reaching one level deeper via child.querySelector('div') to find the actual
// div to tag data-nyancat="1" on. That's four separate structural assumptions
// (child presence/ordering, which testid marks the handle, which inline properties
// the handle happens to set, nesting depth) stacked on top of each other, any one
// of which Spotify could invalidate in an ordinary refactor without renaming a
// single data-testid - and unlike a missing selector, a wrong guess wouldn't even
// fail loudly, it'd just tag the wrong div.
// Fix: added readNativeProgressPercent(), which reads the song's playback percent
// straight off Spotify's own --progress-bar-transform CSS var on [data-testid=
// "progress-bar"] - the exact same var/regex pip-gui's own
// readSpotifyProgressBarPercent() already relies on as its most-trusted fallback,
// so this isn't taking on new risk, just consolidating onto the risk that's
// already deemed acceptable elsewhere in the ecosystem. applyNyanFillToNativeBar()
// uses that single number to set --nyan-fill directly on [data-testid=
// "progress-bar-background"], toggling a .nyan-fill-active class so a new,
// higher-specificity CSS rule (the same RAINBOW-over-BG_PATTERN layering technique
// used for the Lyrics+ bar) paints it - no child walking, no tagging, one selector
// and one property read. applyRainbowToFill() itself wasn't deleted: its
// querySelectorAll now excludes :not(.nyan-fill-active), so it still runs as (a)
// the fallback for the song bar specifically when --progress-bar-transform can't be
// read, so this degrades instead of going fully dark, and (b) the ongoing path for
// the volume bar, which has no equivalent CSS var to switch to. Polled every 200ms
// alongside applyNyanToLyricsPlus() rather than added to fillObserver, since a CSS
// custom property changing is invisible to a childList/subtree MutationObserver the
// same way Lyrics+'s .value property writes already were.
...
(function() {
    ...
    function injectBaseCSS() {
        ...
        style.textContent = `
            ...
            /* Native song progress bar fill, read from Spotify's own
               --progress-bar-transform CSS var (see readNativeProgressPercent())
               instead of walking/tagging child divs. Only applied once
               applyNyanFillToNativeBar() confirms that var is readable - the
               .nyan-fill-active class gate keeps this from fighting the legacy
               applyRainbowToFill() fallback below for the same element. Higher
               specificity than the plain attribute selector above so it always
               wins when active. */
            [data-testid="progress-bar-background"].nyan-fill-active {
                --nyan-fill: 0%;
    ...
    /**
     * readNativeProgressPercent()
     * Reads the current song's playback percent straight off Spotify's own
     * --progress-bar-transform CSS var (same var/regex pip-gui's
     * readSpotifyProgressBarPercent() already trusts as its most durable
     * fallback) instead of inferring it from child-div widths. Returns null if
     * the var isn't present/parseable, so callers can fall back gracefully.
     * @returns {number|null} Percentage (0-100) or null if unavailable
     */
    function readNativeProgressPercent() {
        try {
            const progressBar = document.querySelector('[data-testid="progress-bar"]');
            if (!progressBar) return null;
            const computedStyle = window.getComputedStyle(progressBar);
            const transformVar = computedStyle.getPropertyValue('--progress-bar-transform');
            if (!transformVar) return null;
    ...
    /**
     * applyNyanFillToNativeBar()
     * Keeps the native song bar's --nyan-fill in sync the same way
     * applyNyanToLyricsPlus() does for the Lyrics+ bar: read one exposed
     * percent, write one custom property, let CSS do the painting. If
     * --progress-bar-transform isn't readable (Spotify changed/removed it),
     * removes the .nyan-fill-active class so applyRainbowToFill()'s legacy
     * children-walk picks this bar back up instead of leaving it unstyled.
     */
    function applyNyanFillToNativeBar() {
        const progressBar = document.querySelector('[data-testid="progress-bar"]');
        if (!progressBar) return;
        const bg = progressBar.querySelector('[data-testid="progress-bar-background"]');
        if (!bg) return;

        const pct = readNativeProgressPercent();
        if (pct === null) {
            bg.classList.remove('nyan-fill-active');
            return;
        }
        bg.classList.add('nyan-fill-active');
        bg.style.setProperty('--nyan-fill', pct + '%');
    }
    ...
    function applyRainbowToFill() {
        ...
        // Bars already on the --nyan-fill route (applyNyanFillToNativeBar())
        // are skipped here - this walk is now only a fallback for the song
        // bar (if --progress-bar-transform ever becomes unreadable) and the
        // ongoing path for the volume bar, which has no equivalent CSS var.
        const bars = document.querySelectorAll('[data-testid="progress-bar-background"]:not(.nyan-fill-active)');
    ...
    // same cadence as Lyrics+'s own playback sync. The native bar's
    // --progress-bar-transform var is likewise not something a childList/
    // subtree MutationObserver would ever see change, so it's polled the same
    // way rather than added to fillObserver above.
    setInterval(() => {
        applyNyanFillToNativeBar();
        applyNyanToLyricsPlus();
    }, 200);
